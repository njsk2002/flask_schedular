...
class ImageToBytes:
    ...
    def _pack_bwrybg_4bpp_direct(img: Image.Image,
        ...
        바이트 구성: 상위 nibble=왼쪽 픽셀, 하위 nibble=오른쪽 픽셀.
        가로 픽셀 수가 홀수면 마지막 1픽셀은 'W'로 패딩.  # 2025-09-11
        rtl=True 이면 하드웨어(GDEP133C02) 정합을 위해 각 행의 바이트를 오른쪽→왼쪽 순서로 기록.  # 2025-09-11
        """
        img = img.convert("RGB")
        W, H = img.size
        mp = mapping_nibble or ImageToBytes.MAPPING_DEVICE_6_4BPP

        # 좌→우로 nibble을 만든 뒤, 필요시 바이트 배열을 행 단위로 뒤집음(rtl)
        ...
        for y in range(H):
            ...
            # 하드웨어가 행을 오른쪽부터 소비하면 바이트 배열을 반전
            if rtl:
                row_bytes.reverse()
    ...
    def image_to_payload(img: Image.Image,
        ...
        if mode_up == "BW":
            ...
            raw = ImageToBytes._pack_bwrybg_4bpp_direct(img, mapping_nibble=None)
