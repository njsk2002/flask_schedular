from .. import db
from ..models import Question, Answer, QuestionVoter,  AnswerVoter, YoutubeURL #table 이름
...
class RepositoryYoutube:
    ...
    def insert_utube_url(star_name, type_video, json_file, json_data):
        """
        ...
        # 새로운 일정 데이터 생성
        utube_video = YoutubeURL(
            star_name=star_name,
            type_video=type_video,
            json_file=json_file,
            title_video = json_data["videotitle"],
            url=json_data["url"],
    ...
    def read_utube_url(star_name=None, type_video=None):
        """
        ...
        """
        try:
            query = db.session.query(YoutubeURL)
    ...
    # 중복체크 
    def check_duplication_url(url=None, update_date=None):
            """
            조건에 따라 YouTube URL 데이터를 조회하는 함수.
