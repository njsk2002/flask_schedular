...
from .. import db
from ..forms import CommentForm # forms.py에 있는 class 이름 ==> html의 form형식정의

from ..models import Question, Comment , Answer  #table 이름
...
def create_question(question_id):
    form = CommentForm() # 폼 객체를 생성할 때 괄호를 빠뜨리지 않도록 주의합니다.
...
def delete_question(comment_id):
    ...
    if g.user != comment.user:
        flash("삭제할 권한이 없습니다.")
        return redirect('{}#comment_{}'.format(url_for('question.detail', question_id=question_id),comment_id))
