...
def stock_detail():
    ...
    stockcode = stockcode1.replace('A','') # 한투는 숫자 6자리만 인식함으로 앞자리 삭제
    ...
    #print("스탁정보 :", stock_info[0].stockname)
...
def observe_fromdaily():
    ...
    if request.method == 'POST':
        ...
        # 문자열을 datetime 객체로 다시 변환합니다. 필요한 경우에만 사용합니다.
        today_date_dt = datetime.strptime(today_date_str, "%Y%m%d")
        today_date_dh = datetime.now().strftime('%Y%m%d %H:%M:%S')
...
# AJAX 비동기 ######  데이터DB20버튼 누르면 REALDATA 저장된 DB에서 데이터 받아옴
