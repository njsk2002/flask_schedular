...
class RealTimeData():
    ...
    async def connect(self):
        ...
        g_appkey = "PSol2l0pt8wdI6ZJR9mQTpyHtyHFTKKPxqeC"
        g_appsceret = "waUHuVTB44bbG+5c1X1noqv9QH5dbRIrVhEo+peVqfDWabwQtPcg7ckSpEMWN5/TKJOJuY4SwSCuRbrI5GK6RPJPzU2lpHZbURhLQmo399PsjHbbkOrIkqmJXLjwa1rS6piFoZIJ8o78bJnR9ZBiKaP4s5Jy7+OvNlVJ/50HPd/gdlncFvo="
        
        g_approval_key = self.get_approval(g_appkey, g_appsceret)
                ...
                try:
                    ...
                    if data[0] == '0':
                        ...
                        if trid0 == "H0STASP0":  # 주식호가tr 일경우의 처리 단계
                            print("#### 주식호가 ####")
                            self.stockhoka_domestic(recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0STCNT0":  # 주식체결 데이터 처리
                            print("#### 주식체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_domestic(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)
                            
                        elif trid0 == "H0STMBC0":  # 국내주식 실시간회원사 데이터 처리
                            print("#### 국내주식 실시간회원사 ####")
                            data_cnt = int(recvstr[2])  # 데이터 개수
                            self.stocksmember_domestic(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2) 

                        elif trid0 == "H0STPGM0":  # 국내주식 실시간프로그램매매 데이터 처리
                            print("#### 국내주식 실시간프로그램매매 ####")
                            data_cnt = int(recvstr[2])  # 데이터 개수
                            self.stocksprogramtrade_domestic(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)                        
                            
                        elif trid0 == "H0STMKO0":  # 국내주식 장운영정보 데이터 처리
                            print("#### 국내주식 장운영정보 ####")
                            data_cnt = int(recvstr[2])  # 데이터 개수
                            self.stocksmarketinfo_domestic(data_cnt, recvstr[3])  
                            
                        elif trid0 == "H0UPCNT0":  # 국내지수 체결 데이터 처리
                            print("#### 국내지수 체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.indexpurchase_domestic(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2) 
                            
                        elif trid0 == "H0UPANC0":  # 국내지수 예상체결 데이터 처리
                            print("#### 국내지수 예상체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.indexexppurchase_domestic(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2) 
                            
                        elif trid0 == "H0UPPGM0":  # 국내지수 예상체결 데이터 처리
                            print("#### 국내지수 실시간프로그램매매 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.indexprogramtrade_domestic(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)    

                        elif trid0 == "HDFSASP0":  # 해외주식호가tr 일경우의 처리 단계
                            print("#### 해외(미국)주식호가 ####")
                            self.stockhoka_overseas(recvstr[3])
                            # await asyncio.sleep(0.2)
                            ...
                            print("#### 해외주식체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_overseas(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0IFASP0":  # 지수선물호가 tr 일경우의 처리 단계
                            print("#### 지수선물호가 ####")
                            self.stockhoka_futs(recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0IFCNT0":  # 지수선물체결 데이터 처리
                            print("#### 지수선물체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_futs(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0IOASP0":  # 지수옵션호가 tr 일경우의 처리 단계
                            print("#### 지수옵션호가 ####")
                            self.stockhoka_optn(recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0IOCNT0":  # 지수옵션체결 데이터 처리
                            print("#### 지수옵션체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_optn(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)
                        
                        elif trid0 == "H0CFASP0":  # 상품선물호가 tr 일경우의 처리 단계
                            print("#### 상품선물호가 ####")
                            self.stockhoka_productfuts(recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0CFCNT0":  # 상품선물체결 데이터 처리
                            print("#### 상품선물체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_productfuts(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)
                            
                        elif trid0 == "H0ZFCNT0":  # 주식선물 체결 데이터 처리
                            print("#### 주식선물 체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_stockfuts(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2) 

                        elif trid0 == "H0ZFASP0":  # 주식선물 호가 데이터 처리
                            print("#### 주식선물 호가 ####")
                            self.stockhoka_stockfuts(recvstr[3])
                            # await asyncio.sleep(0.2)
                            
                        elif trid0 == "H0ZOCNT0":  # 주식옵션 체결 데이터 처리
                            print("#### 주식옵션 체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_stockoptn(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2) 

                        elif trid0 == "H0ZOASP0":  # 주식옵션 호가 데이터 처리
                            print("#### 주식옵션 호가 ####")
                            self.stockhoka_stockoptn(recvstr[3])
                            # await asyncio.sleep(0.2)     

                        elif trid0 == "HDFFF010":  # 해외선물옵션호가 tr 일경우의 처리 단계
                            print("#### 해외선물옵션호가 ####")
                            self.stockhoka_overseafut(recvstr[3])
                            # await asyncio.sleep(0.2)

                        elif trid0 == "HDFFF020":  # 해외선물옵션체결 데이터 처리
                            print("#### 해외선물옵션체결 ####")
                            data_cnt = int(recvstr[2])  # 체결데이터 개수
                            self.stockspurchase_overseafut(data_cnt, recvstr[3])
                            # await asyncio.sleep(0.2)
                        ...
                        if trid0 == "H0STCNI0" or trid0 == "H0STCNI9":  # 주실체결 통보 처리
                            self.stocksigningnotice_domestic(recvstr[3], aes_key, aes_iv)
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0GSCNI0":  # 해외주실체결 통보 처리
                            self.stocksigningnotice_overseas(recvstr[3], aes_key, aes_iv)
                            # await asyncio.sleep(0.2)

                        elif trid0 == "H0IFCNI0":  # 지수/상품/주식 선물옵션체결 통보 처리
                            self.stocksigningnotice_futsoptn(recvstr[3], aes_key, aes_iv)
                            # await asyncio.sleep(0.2)

                        elif trid0 == "HDFFF2C0":  # 해외선물옵션체결 통보 처리
                            self.stocksigningnotice_overseafut(recvstr[3], aes_key, aes_iv)
                            # await asyncio.sleep(0.2)
